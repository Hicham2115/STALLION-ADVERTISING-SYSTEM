...
router.put(
  ...
  h(async (req, res) => {
    ...
    const wasShipped =
      existing.status !== "SHIPPED" && data.status === "SHIPPED";
      ...
      if (!alreadyExists) {
        await (prisma as any).closerCommissionRecord.create({
          data: {
            closerId,
            orderId: order.id,
            amount: Number(order.closerCommission),
          },
        });
      }
    }
...
router.get(
  "/my-orders",
  h(async (req, res) => {
    const { status, search, page = "1", from, to } = req.query;
    const take = 30;
  ...
  h(async (req, res) => {
    ...
    const [total, confirmed, shipped, delivered, pendingComm, totalComm] =
      await Promise.all([
        ...
        (prisma as any).crmOrder.count({
          where: { closerId: userId, status: "SHIPPED", ...dateWhere },
        }),
        (prisma as any).crmOrder.count({
          where: { closerId: userId, status: "DELIVERED", ...dateWhere },
        }),
        (prisma as any).closerCommissionRecord.aggregate({
          where: { closerId: userId, paid: false, order: { ...dateWhere } },
          _sum: { amount: true },
        }),
        (prisma as any).closerCommissionRecord.aggregate({
          where: { closerId: userId, order: { ...dateWhere } },
          _sum: { amount: true },
        }),
      ]);
    res.json({
  ...
  h(async (req, res) => {
    ...
    res.json({
      summary: {
        ...
        totalAdSpend,
        totalProductCost,
        totalLinkedCosts: linkedCostSpend,
        totalShipping,
        confirmed,
        shipped,
        delivered,
