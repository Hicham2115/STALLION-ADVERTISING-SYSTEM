...
router.get('/overview', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const result = await Promise.all(agencies.map(async (agency) => {
    const superAdmin = agency.users.find((u) => u.role === 'SUPER_ADMIN');
    const teamCount = agency.users.filter((u) => u.role !== 'SUPER_ADMIN').length;
    const activeClients = agency.clients.filter((c) => !c.archived).length;
    const openLeads = agency.leads.filter((l) => !['WON', 'LOST'].includes(l.status)).length;
    const wonLeads = agency.leads.filter((l) => l.status === 'WON').length;

    // Revenue: sum all payments for this agency
    ...
    const months = Array.from({ length: 6 }, (_, i) => {
      ...
      return { label: d.toLocaleString('en-US', { month: 'short' }), start: new Date(d.getFullYear(), d.getMonth(), 1), end: new Date(d.getFullYear(), d.getMonth() + 1, 0, 23, 59, 59) };
    });
